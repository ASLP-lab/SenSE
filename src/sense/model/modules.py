...
def get_bigvgan_qwen_mel_spectrogram(
    waveform,
    n_fft=1024,
    n_mel_channels=100,
    target_sample_rate=24000,
    ...
    # Check range
    if torch.min(waveform) < -1.0 or torch.max(waveform) > 1.0:
        print(f"[Warning] Waveform out of range: min={torch.min(waveform)}, max={torch.max(waveform)}")
    ...
    win_key = f"{win_length}_{dtype_device}"
    ...
    if mel_key not in mel_basis_cache:
        ...
        mel_basis_cache[mel_key] = torch.from_numpy(mel_filter).to(dtype=waveform.dtype, device=waveform.device)

    # Create Hann window if not cached
    if win_key not in hann_window_cache:
        hann_window_cache[win_key] = torch.hann_window(win_length).to(dtype=waveform.dtype, device=waveform.device)
    
    mel_basis = mel_basis_cache[mel_key]
    hann_window = hann_window_cache[win_key]
    ...
    mel_spec = torch.matmul(mel_basis, magnitude)  # [B, mel, T]
...
def get_conformer_mel_spectrogram(
    waveform,
    n_fft=400,
    n_mel_channels=80,
    target_sample_rate=24000,
...
def get_whisper_mel_spectrogram(
    waveform,
    n_fft=None,
    n_mel_channels=None,
    ...
    assert len(waveform.shape) == 2
    audio_mel = preprocess_audio(waveform)
    return audio_mel
...
class MelSpec(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert mel_spec_type in ["vocos", "bigvgan", "bigvgan_qwen", "conformer", "whisper"], print("We only support four extract mel backend: vocos, bigvgan, bigvgan_qwen or conformer")
        ...
        if mel_spec_type == "vocos":
            ...
            self.extractor = get_bigvgan_qwen_mel_spectrogram
        elif mel_spec_type == "conformer":
            self.extractor = get_conformer_mel_spectrogram
        elif mel_spec_type == "whisper":
            self.extractor = get_whisper_mel_spectrogram
