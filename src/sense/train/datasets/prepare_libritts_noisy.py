...
def deal_with_audio_dir(audio_dir):
    ...
    for line in audio_lists:
        ...
        if duration < 0.4 or duration > 30:
            continue
        sub_result.append({"audio_path": str(line), "noisy_audio_path": str(noisy_path), "text": text, "duration": duration})
...
if __name__ == "__main__":
    ...
    dataset_dir = "/home/node25_tmpdata/data/LibriTTS/LibriTTS_noisy"
    dataset_name = f"LibriTTS_noisy_{'_'.join(SUB_SET)}_{tokenizer}".replace("train-clean-", "").replace("train-other-", "")
