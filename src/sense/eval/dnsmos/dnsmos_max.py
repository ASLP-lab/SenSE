...
def extract_base_name(filename):
    """提取音频文件的基础名称，去掉_sample1.wav等后缀"""
    # 匹配 _sample1.wav 到 _sample10.wav 的模式
    pattern = r'_sample\d+\.wav$'
    base_name = re.sub(pattern, '', filename)
    return base_name

def group_files_by_base_name(clips):
    """将文件按基础名称分组"""
    grouped_files = defaultdict(list)
    for clip in clips:
        base_name = extract_base_name(os.path.basename(clip))
        grouped_files[base_name].append(clip)
    return grouped_files

def get_max_scores_for_group(group_data):
    """获取一组文件中各指标的最大值"""
    if not group_data:
        return None
    
    max_scores = {
        'filename': group_data[0]['filename'],  # 使用第一个文件的完整路径作为标识
        'len_in_sec': 'N/A',
        'sr': SAMPLING_RATE,
        'num_hops': max([d['num_hops'] for d in group_data]),
        'OVRL_raw': max([d['OVRL_raw'] for d in group_data]),
        'SIG_raw': max([d['SIG_raw'] for d in group_data]),
        'BAK_raw': max([d['BAK_raw'] for d in group_data]),
        'OVRL': max([d['OVRL'] for d in group_data]),
        'SIG': max([d['SIG'] for d in group_data]),
        'BAK': max([d['BAK'] for d in group_data]),
        'P808_MOS': max([d['P808_MOS'] for d in group_data])
    }
    return max_scores
...
def main(args):
    ...
    # 按基础名称分组
    grouped_files = group_files_by_base_name(clips)
    
    # 为每个组计算最大值
    for base_name, file_list in grouped_files.items():
        # 找到这个组对应的所有结果
        group_results = [result for result in all_results if extract_base_name(os.path.basename(result['filename'])) == base_name]
        if group_results:
            max_scores = get_max_scores_for_group(group_results)
            if max_scores:
                rows.append(max_scores)
...
def calculate_dnsmos_score_max(testset_dir, dnsmos_path, csv_path=None, json_path=None):
    """计算DNSMOS分数，对相同基础名称的文件取最大值"""
...
if __name__=="__main__":
    ...
    parser.add_argument('-d', "--dnsmos_path", default='dnsmos/DNSMOS', help='Path to the DNSMOS model directory')
