...
if __name__=="__main__":
    parser = argparse.ArgumentParser(
        description="Calculate intrusive evaluation metrics for speech enhancement (PESQ, ESTOI)"
    )
    parser.add_argument(
        "--testset_dir",
        required=True,
        help="Test audio files directory path (enhanced audio)"
    )
    parser.add_argument(
        "--gt_dir", 
        required=True,
        help="Ground truth/reference audio files directory path (clean speech)"
    )
    parser.add_argument(
        "--json_path",
        required=True,
    ...
    parser.add_argument(
        "--csv_path",
        help="Optional: Output CSV detailed results file path"
    )
    
    args = parser.parse_args()
    
    print(f"Test audio directory: {args.testset_dir}")
    print(f"Reference audio directory: {args.gt_dir}")
    print(f"JSON output path: {args.json_path}")
    if args.csv_path:
        print(f"CSV output path: {args.csv_path}")
    print("-" * 60)
    
    # Call calculation function
    calculate_intrusive_score(
        gt_dir=args.gt_dir,
